// app.jsx — Root + routing + global state (dark mode, language, settings)
...
const App = () => {
  ...
  // Apply dark mode class to the app shell div (scoped to phone, not page)
  ...
  const handleCreate = (data) => {
    ...
    const next = {
      id, ...data,
      progress: 0, answered: 0, total: 144,
      status: 'in-progress', workflow: 'Pre-inspection', sync: 'pending',
    };
    setInspections([next, ...inspections]);
    setNewOpen(false);
    setOpenId(id);
  };
  ...
  const handleComplete = () => {
    ...
    setInspections(inspections.map(i => i.id === openId
      ? { ...i, status: 'completed', progress: 1, workflow: 'Submitted' } : i));
  };
  ...
  const renderTab = () => {
    ...
    if (tab === 'forum') return <ForumScreen />;
  ...
  return (
    <div style={{
      ...
      {/* Status bar */}
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 99 }}>
        <IOSStatusBar />
      </div>

      <div className={shellClass}>
        {renderTab()}
        <BottomNav active={tab} onChange={(t) => { setOpenId(null); setTab(t); }} onAi={() => setAiOpen(true)} />
      </div>
